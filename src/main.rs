...
/*use main::public;
use settings::{new_logger_settings, MainConfiguration};
...
async fn main() {
    ...
    if configuration_settings.heartbeat_logging {        
        ...
        tokio::spawn(async move {
            let mut time = interval(Duration::from_secs(configuration_settings.heartbeat_interval_hours as u64 * 60 * 60));
    ...
    match public::init(configuration_settings).await {
        Ok(_) => {
...
pub mod public {
    ...
    pub async fn init(config_settings: MainConfiguration) -> Result<(), String> {
        return main(config_settings).await;
    }
}

mod main {
    use crate::login::login_handler;
    use crate::callback::callback_handler;
    use crate::sessionstatus::sessionstatus_handler;
    use crate::details::user_details_handler;
    ...
    // Start the server
    pub(super) async fn main(config_settings: MainConfiguration) -> Result<(), String> {
